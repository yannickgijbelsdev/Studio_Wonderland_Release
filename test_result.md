...
backend:
  ...
  - task: "News proxy API (GET list + GET article by id) to external Clara/koodh CMS"
    ...
    status_history:
        ...
        -working: true
        -agent: "testing"
        -comment: "✅ All 4 News API tests passed (100% success)."
        -working: "NA"
        -agent: "main"
        -comment: "USER BUG (production): gallery photos don't show on production — only hard-coded fallback photos. RCA: the /news proxy ran AFTER connectToMongo() (called at top of handleRoute for every route). If the production Mongo connection is slow/fails, the whole handler throws 500 before the news fetch runs, and a rejected clientPromise was cached forever (never retried), permanently breaking /news → frontend falls back to hard-coded PHOTOS. FIX: (1) extracted the news proxy into handleNews() and now handle all /news routes BEFORE connectToMongo(), so news/gallery is fully independent of MongoDB. (2) connectToMongo now resets clientPromise to undefined on connection failure so it can retry on the next request. Note: Sinterklaas galerij (site=sinterklaas-genk, category=galerij) returns 7 real photos with https URLs; Kerst galerij is currently empty (fallback is correct there)."
        -working: true
        -agent: "testing"
        -comment: "✅ ALL 7 TESTS PASSED (100% success) - MongoDB decoupling refactor verified. CRITICAL TEST #1 (Sinterklaas gallery): ✅ GET /api/news?category=galerij returns 200 with exactly 7 items, each with valid https image_url from nbg1.your-objectstorage.com/koodh-clara (NOT empty, NOT fallback photos). TEST #2: ✅ GET /api/news?site=het-huis-van-de-kerstman&category=nieuws returns 200 with items[] (0 items, valid). TEST #3: ✅ GET /api/news?site=studio-wonderland&category=eerder-te-beleven returns 200 with items[] (0 items, valid). TEST #4: ✅ GET /api/news/{id} returns 200 with full article including body field (HTML), title, image_url, published_at, category. TEST #5: ✅ GET /api/news/nonexistent-id-123 returns 404 with error field 'Artikel niet gevonden', server did NOT crash. SANITY CHECKS: ✅ GET /api/productions (200, 3 items) and ✅ GET /api/contact (200, 1 item) - no regression, Mongo-backed routes still work after refactor. The core production bug is FIXED: news/gallery endpoints now work independently of MongoDB connection status. The handleNews() function executes BEFORE connectToMongo() in the request flow, and connectToMongo() now resets failed clientPromise for retry. Gallery photos are successfully fetched from external CMS."
...
test_plan:
  current_focus:
    - "News proxy API (GET list + GET article by id) to external Clara/koodh CMS"
  stuck_tasks: []
  test_all: false
...
agent_communication:
    ...
    -agent: "main"
    -agent: "testing"
    -message: "✅ NEWS PROXY MONGODB DECOUPLING REFACTOR VERIFIED - ALL 7 TESTS PASSED (100%). Tested the production gallery bug fix where photos showed only hard-coded fallbacks due to slow/failed Mongo connection blocking /news proxy. CRITICAL TEST #1 (Sinterklaas gallery): ✅ GET /api/news?category=galerij returns HTTP 200 with exactly 7 items, each with valid https image_url from nbg1.your-objectstorage.com/koodh-clara (real photos from external CMS, NOT empty, NOT fallback). TEST #2: ✅ GET /api/news?site=het-huis-van-de-kerstman&category=nieuws returns 200 with items[] (0 items currently, valid structure). TEST #3: ✅ GET /api/news?site=studio-wonderland&category=eerder-te-beleven returns 200 with items[] (0 items, valid). TEST #4: ✅ GET /api/news/{id} returns 200 with full article including body (HTML), title, image_url, published_at, category. TEST #5: ✅ GET /api/news/nonexistent-id-123 returns 404 with error field, server did NOT crash. REGRESSION SANITY: ✅ GET /api/productions (200, 3 items) and ✅ GET /api/contact (200, 1 item) - Mongo-backed routes still work correctly after refactor. REFACTOR VERIFICATION: Confirmed handleNews() executes BEFORE connectToMongo() in app/api/[[...path]]/route.js (lines 121-124), making news/gallery fully independent of MongoDB. connectToMongo() now resets failed clientPromise to undefined (line 17) allowing retry on next request. The production bug is FIXED - gallery photos are successfully fetched from external CMS regardless of MongoDB connection state."
    -message: "RETEST NEWS PROXY ONLY (production gallery bug). User reports on PRODUCTION the gallery shows only hard-coded fallback photos (real Koodh photos missing). I refactored app/api/[[...path]]/route.js so the /news proxy is now handled BEFORE connectToMongo() (fully independent of MongoDB), and connectToMongo() now clears a failed clientPromise so it can retry. Please verify (base path /api): (1) GET /api/news?category=galerij — must return 200 with items[] containing 7 real photos (each item has image_url that is an https URL from your-objectstorage.com). This is the Sinterklaas gallery that must NOT be empty. (2) GET /api/news?site=het-huis-van-de-kerstman&category=nieuws — 200 with items[] (Christmas news). (3) GET /api/news?site=studio-wonderland&category=eerder-te-beleven — 200 (object with items[], may be empty). (4) GET /api/news/{id} using an id from the Sinterklaas galerij or a valid article — 200 with full article (title, image_url, body). (5) GET /api/news/nonexistent-id-123 — non-200 (404/502) with error, no crash. (6) Sanity: GET /api/productions (200) and GET /api/contact (200) still work (Mongo-backed) — confirms the refactor didn't break DB routes. Focus especially on #1 returning 7 non-empty items — that's the reported bug."